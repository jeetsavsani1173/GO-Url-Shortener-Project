...
import (
	...
	"github.com/jeetsavsani1173/shorten-url-fiber-redis/helpers"
)
...
func ShortenURL(c *fiber.Ctx) error {
	...
	// with expiry of `30mins`. So in this case the user will be able to send 50
	// requests every 30 minutes
	r2 := database.CreateClient(1)
	defer r2.Close()
	val, err := r2.Get(database.Ctx, c.IP()).Result()

	if err == redis.Nil { // it means that IP adderess entry is not present in our DB.
		//change the rate_limit_reset here, change `30` to your number
	...
	// haven't performed any collision checks on this but will add in future
	var id string
	if body.CustomShort == "" {
	...
	if val != "" {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
			"message": "URL custom short is already in use",
		})
	}
